...
def get_r_obj(data, obj):
    ...
    out_dist = [[i.rect.centerx - obj.rect.centerx, i.rect.centery - obj.rect.centery] for i in
                   dist.values()]  # преобразования для подачи в нейросеть(вектора к объекту от наименьшего расстояния)
    return out_dist
...
class Bact(pg.sprite.Sprite):
    def __init__(self, x, y, weight):
        ...
        pg.draw.circle(self.orig_image, color_bact_herb, (cell_bact // 2, cell_bact // 2), cell_bact // 2)  # тело бактерии
    ...
    def update(self, bacts_herb, bacts_omn, foods):
        ...
        bact_collide = pg.sprite.Group(i for i in bacts_herb)
